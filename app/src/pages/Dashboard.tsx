            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {statCards.map((stat, index) => (
                    <div
                        ...
                        className={`glass-card p-6 lg:p-8 group h-full flex flex-col justify-between cursor-pointer hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 shadow-xl`}
                    >
                        <div className="flex items-center justify-between gap-4 mb-6 relative z-10 text-muted">
                            <p className="text-[9px] font-black uppercase tracking-[0.2em] truncate flex-1">{stat.label}</p>
                            <div className={`w-10 h-10 flex items-center justify-center rounded-2xl ${stat.bgClass} backdrop-blur-xl transition-all duration-500 border flex-shrink-0 group-hover:scale-110 group-hover:rotate-6`}>
                                <stat.icon
                        ...
                        <div className="flex flex-col gap-1 relative z-10">
                            <div className="flex items-baseline justify-between mb-1">
                                <h3 className="text-4xl font-black text-base tracking-tighter">
                                    {loading ? (
                                        <div className="h-10 w-24 bg-surface-border/50 animate-pulse rounded-xl"></div>
                                    ) : (
                                        stat.value
                                    )}
                                </h3>
                                <ArrowUpRight className="w-5 h-5 text-muted group-hover:text-primary group-hover:translate-x-1 group-hover:-translate-y-1 transition-all duration-300" />
                            </div>
                            <div className={`flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.1em] text-muted mt-2`}>
                                {stat.isLive ? (
                    ...
                    <div className="p-6 space-y-3">
                        {loading ? (
                            ...
                            displayStats.recentActivity.map((student: any) => (
                                <div key={student.id} className="flex items-center gap-3 p-4 bg-surface-border/10 rounded-[1.5rem] border border-surface-border hover:bg-surface-border/20 transition-all duration-300 group overflow-hidden">
                                    <div className="w-10 h-10 rounded-xl bg-primary/10 text-primary flex items-center justify-center font-black group-hover:scale-110 transition-transform flex-shrink-0">
                                        {student.full_name.charAt(0)}
                                    </div>
                                    <div className="min-w-0 flex-1">
                                        <p className="font-extrabold text-base group-hover:text-primary transition-colors uppercase tracking-tight leading-tight mb-1 whitespace-nowrap overflow-hidden pr-4" style={{ maskImage: 'linear-gradient(to right, black calc(100% - 32px), transparent 100%)', WebkitMaskImage: 'linear-gradient(to right, black calc(100% - 32px), transparent 100%)' }}>{student.full_name}</p>
                                        <div className="flex items-center justify-between gap-3 w-full">
                                            ...
                                            <span className="inline-flex items-center px-2 py-0.5 rounded-lg text-[8px] font-black uppercase tracking-[0.2em] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 shadow-lg shadow-emerald-500/5 shrink-0">{t('students.active')}</span>
                                        </div>
                                    </div>
                                </div>
                            ))
